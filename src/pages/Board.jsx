...
//       const startPostId = (pageNumber - 1) * 6 + 1; 
//       try {
//         const response = await axios.post('http://192.168.0.13:8080/board/apiTest', startPostId.toString());
//         if (response.data && Array.isArray(response.data.items)) {
//           setRecipes(response.data.items.map(item => ({
//             postid: item.ID,
//             title: item.title,
//             description: item.Recipe,
//             img: item.thumbnail,
...
//           })));
//           setTotalPages(Math.ceil(response.data.total / 6)); // 전체 페이지 수 계산 및 설정
//         }
//       } catch (error) {
