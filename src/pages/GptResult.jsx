  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-screen">
        ...
        <button onClick={() => navigate('/main')} className="ml-4 text-2xl">
          <GoHome />
        </button>
      </div>
    );
    ...
    <div className="bg-white min-h-screen p-6">
            ...
            <div className="recipebox p-4 bg-gray-100 rounded-lg overflow-y-auto max-h-96">
              ...
              <ul className="py-2 flex flex-wrap"> 
                {ingredients.map((ingredient, index) => (
                  <li key={index} className="font-score text-gray-600 mr-4 mb-2">
                    {ingredient}
                  </li>
                ))}
              </ul>
      ...
      <div className="fixed bottom-5 left-0 right-0 px-6">
        <div className="mx-auto flex gap-14" style={{ maxWidth: '400px' }}>
          ...
          <button className="font-score bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-8 rounded-full" onClick={handleSaveButtonClick}>
            저장할래요
          </button>
        </div>
      </div>
